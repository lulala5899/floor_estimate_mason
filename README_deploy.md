...
## 7. 不方便反复坐电梯？用假数据脚本纯软件测试防抖逻辑

如果你只是想验证"防抖参数设置得合不合理"，不需要每次都真的移动设备——可以写一个
脚本，假装自己是 ESP32，往串口发送符合协议格式的假数据，专门测试边界情况（比如让
气压在楼层分界线附近来回抖动，看 `/floor_state` 会不会跟着乱跳）。
...
A: 给其他模块用请用 `/floor_state`（已降噪、稳定后才更新）。`/floor_estimate` 只用来
调试或需要"越快越好、允许偶尔跳动"的场景。
